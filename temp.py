...
class DCGAN(object):
    def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
                 ...
                 input_fname_pattern='*.webp', checkpoint_dir=None, sample_dir=None):
        """
    ...
    def train(self, config):
        ...
        if config.dataset == 'mnist':
            ...
            data = glob(os.path.join(config.filepath, self.input_fname_pattern),
                        recursive=True)
        ...
        for epoch in range(config.epoch):
            if config.dataset == 'mnist':
                ...
                data = glob(os.path.join("/media/mridul/New Volume/data2/**/", self.input_fname_pattern),
                            recursive=True)
            ...
            for idx in range(0, batch_idxs):
                if config.dataset == 'mnist':
                    ...
                    for batch_file in batch_files:
                        cat, image = get_image(batch_file, input_height=self.input_height, input_width=self.input_width,
                                               resize_height=self.output_height, resize_width=self.output_width,
                                               is_crop=self.is_crop, is_grayscale=self.is_grayscale)
                        batch.append(image)
                        batch_y.append([cat])
